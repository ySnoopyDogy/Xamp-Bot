...
class MessageConfigCommand extends Command {
  ...
  async run(ctx) {
    ...
    const embed = new MessageEmbed()
      ...
      **Canal de Criação de Ticket ES**: <#${allConfigs.messageTicketES ?? '`NONE`'}> (${allConfigs.messageTicketES ?? '`NONE`'})
      ...
      **Cargo do Idioma ES**: <@&${allConfigs.roleLangES ?? '`NONE`'}> (${allConfigs.roleLangES ?? '`NONE`'})
